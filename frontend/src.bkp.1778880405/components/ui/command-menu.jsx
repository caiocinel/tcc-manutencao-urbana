...
const pages = [
  ...
  { path: '/admin', icon: Layout, label: 'Painel Admin', keywords: 'admin painel administracao', admin: true },
  { path: '/admin/dashboard', icon: ChartBar, label: 'Métricas e Dashboard', keywords: 'metricas dashboard graficos admin', admin: true },
  { path: '/admin/usuarios', icon: Users, label: 'Gerenciar Usuários', keywords: 'usuarios admin administradores', admin: true },
  { path: '/configuracoes', icon: GearSix, label: 'Configurações Gerais', keywords: 'configuracoes gerais sistema' },
];
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-[15%] left-1/2 -translate-x-1/2 z-[5001] w-full max-w-lg"
          >
            <div
            ...
            >
              <div className="flex items-center gap-3 px-4 h-12 border-b" style={{ borderColor: 'var(--color-border-default)' }}>
                <MagnifyingGlass size={18} style={{ color: 'var(--color-text-tertiary)' }} />
                <input
                  ref={inputRef}
                  type="text"
                  value={query}
              ...
              <div className="max-h-72 overflow-y-auto p-2">
                ...
                {filtered.map((item, i) => (
                  <button
                    ...
                    onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; if (i !== selectedIdx) e.currentTarget.style.color = 'var(--color-text-secondary)'; }}
                  >
                    <item.icon size={18} weight={i === selectedIdx ? 'bold' : 'regular'} style={{ color: i === selectedIdx ? 'var(--color-gold-500)' : 'var(--color-text-tertiary)' }} />
