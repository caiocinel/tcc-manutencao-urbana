...
const statusColors = {
  pendente: 'var(--color-gold-500)',
  em_andamento: 'var(--accent-orange)',
...
function ClusterMarker({ c, selectedIds, isAuthenticated, encerrando, onToggleSelect, onConfirmEncerrar, onApoiar, onAttach }) {
  ...
  const clusterIcon = L.divIcon({
    className: 'custom-marker',
    html: `<div style="background:var(--color-gold-500);width:36px;height:36px;border-radius:50%;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.4);display:flex;align-items:center;justify-content:center;color:#000;font-weight:700;font-size:13px">${c.total}</div>`,
    iconSize: [36, 36],
    iconAnchor: [18, 18],
  });
...
export default function MapPage() {
  ...
  const boundaryColor = theme === 'light' ? 'var(--color-gold-500)' : 'var(--color-gold-600)';
